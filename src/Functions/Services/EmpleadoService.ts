...
export default class EmpleadoService extends GenericFetch<IEmpleado> {
  // Agregá esto dentro de tu clase EmpleadoService en el Front
  async saveEmpleadoCompleto(empleadoData: IEmpleado, id?: number): Promise<boolean> {
    const method = id && id !== 0 ? "PUT" : "POST";
    const url = id && id !== 0 ? `${this.baseUrl}/${id}` : this.baseUrl;
  ...
  async getEmpleadobyDenominacion(
    codigo: string,
  ): Promise<IPaginatedResponse<IEmpleado> | null> {
  ...
  async deleteEmpleado(id: number): Promise<void> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Error deleting insumo: ${response.status} - ${errorData?.message || response.statusText}`,
      );
    }
  }
}
